...
import { avaliarAtendimento, ESTADOS_ATENDIMENTO, piorEstado } from '@/lib/operacoes/atendimento';
...
export default async function OperacoesPage({
  ...
  const nivel = piorEstado([
    avaliarAtendimento(painel.atendimento).nivel,
    painel.saude.nivel === 'saudavel' ? 'normal' : painel.saude.nivel,
  ]);
    ...
    <>
      ...
      <main className={styles.pagina}>
        <header className={styles.hero}>
          <div>
            <p>
              <Activity size={18} aria-hidden="true" /> Administração
            </p>
            <h1>Saúde do sistema</h1>
            <span>IA, suporte e processamento.</span>
          </div>
          <div className={styles.sinal} data-nivel={nivel}>
            <strong>{ESTADOS_ATENDIMENTO[nivel]}</strong>
            <form action="/admin/operacoes" method="get">
              <button className={styles.atualizar} type="submit">
                <RefreshCw size={16} aria-hidden /> Atualizar painel
              </button>
            </form>
          </div>
        </header>
        <SaudeAtendimento resumo={painel.atendimento} />
        {!!painel.saude.alertas.length && (
          <div className={styles.alertas} aria-label="Alertas de processamento">
            {painel.saude.alertas.map((alerta) => (
              <article key={alerta.id}>
                ...
                <a href="#historico-operacoes">Ver execuções</a>
              </article>
            ))}
          </div>
        )}
        <details className={styles.grupo}>
          <summary>
            Prospecção, reuniões e custos <span>Últimas 24h</span>
          </summary>
          ...
          <section className={styles.escala} aria-labelledby="titulo-capacidade-operacional">
            <header>
              <div>
                ...
                <h2 id="titulo-capacidade-operacional">Processamento e consumo</h2>
              </div>
        ...
        <details
          ...
          open={Boolean(filtros.status || filtros.tipo)}
        >
          <summary>
            Histórico de execuções <span>{painel.operacoes.length} registros recentes</span>
          </summary>
          <section className={styles.painel} aria-labelledby="titulo-fila-operacional">
            <header>
              <div>
                <h2 id="titulo-fila-operacional">Execuções recentes</h2>
              </div>
