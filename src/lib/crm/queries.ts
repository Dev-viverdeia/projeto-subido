...
export const obterDossieLead = cache(async (id: string): Promise<DossieLead | null> => {
  ...
  const { data: linha, error } = await supabase
    ...
    .select(
      'id, titulo, etapa, empresa_id, contato_principal_id, valor_centavos, revisao_comercial, proxima_acao, proxima_acao_em, ganha_em, perdida_em, motivo_perda, atualizado_em, criado_em, situacao, retirada_em, motivo_retirada',
    )
    .eq('id', id)
    .maybeSingle();
  ...
  const [
    ...
    supabase
      .from('crm_empresas')
      .select(
        'id, nome, dominio, setor, porte, cidade, estado, enriquecido_em, enriquecimento, revisao, cadastro_editado_em, site_manual',
      )
  ...
  const lead: DossieLead = {
    ...
    empresa: {
      cadastroEditadoEm: empresaLinha.cadastro_editado_em,
      siteManual: empresaLinha.site_manual,
      nome: empresaLinha.nome,
      dominio: empresaLinha.dominio,
    ...
    edicaoVenda: { revisao: linha.revisao_comercial },
    edicaoEmpresa: { id: empresaLinha.id, revisao: empresaLinha.revisao },
    edicaoContato: {
