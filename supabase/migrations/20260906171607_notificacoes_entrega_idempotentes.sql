...
-- O conteúdo não é armazenado: só sua impressão digital e o início da janela
-- de idempotência. Nenhuma credencial ou corpo de e-mail é exposto ao cliente.
alter table public.projeto_portal_eventos
  add column email_fingerprint text check (email_fingerprint ~ '^[a-f0-9]{64}$'),
  add column email_primeira_tentativa_em timestamptz;

create function public.projeto_email_reservar(
...
begin
  ...
  inicio := case when e.email_fingerprint = p_fingerprint
...
-- Webhooks podem chegar antes da resposta da API e fora de ordem. A impressão
-- digital impede que o resultado de um envio antigo substitua um endereço corrigido.
...
revoke all on function public.projeto_email_reservar(uuid,text,text,text) from public, anon, authenticated;
revoke all on function public.projeto_email_confirmar(uuid,text,text,text,timestamptz) from public, anon, authenticated;
grant execute on function public.projeto_email_reservar(uuid,text,text,text) to service_role;
grant execute on function public.projeto_email_confirmar(uuid,text,text,text,timestamptz) to service_role;
commit;
